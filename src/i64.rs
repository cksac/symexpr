...
std_bin_op!(I64, Add, add, ValType);
std_bin_op!(I64, Sub, sub, ValType);
std_bin_op!(I64, Mul, mul, ValType);
std_bin_op!(I64, Div, div, ValType);
    ...
    C: SymCtx<ValType>,
{
    ...
    fn eval(&self, ctx: &C) -> Result<Self::Value> {
        match self {
            I64::Const(v) => Ok(*v),
            I64::Symbol(s) => ctx.get(*s),
            I64::Expr(e) => e.eval(ctx),
        }
    }

    fn cloned(&self) -> Box<dyn SymValue<C, Value = Self::Value>> {
        match self {
            I64::Const(v) => Box::new(*v),
            I64::Symbol(s) => Box::new(I64::Symbol(*s)),
            I64::Expr(e) => e.cloned(),
        }
    }
}
...
impl SymCtx<ValType> for I64Ctx {
